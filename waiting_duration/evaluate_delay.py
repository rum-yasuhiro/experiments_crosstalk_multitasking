...
class EvaluateDelay: 
    def __init__(self, job_sim, job_delay_op, job_delay_meas, delay_duration_list, initial_layout):
    ...
    def js_divergence(self, counts_list_list, label):
        jsd_list_list = []
        for counts_list in counts_list_list: 
            jsd_list = [jsd(counts_i, self.counts_sim, self.num_clbits) for counts_i in counts_list]
            jsd_list_list.append(jsd_list)

        # standard mean
        mean_list = statistics.mean(jsd_list_list)
        # standard error mean
        sem_list = statistics.sem(jsd_list_list)
